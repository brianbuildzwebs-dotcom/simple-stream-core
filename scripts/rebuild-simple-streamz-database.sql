...
-- BUILD_ID: 2026-06-30-v3  (confirm this line is present before running)
...
create table if not exists public.trial_abuse (
  id                  uuid primary key default gen_random_uuid(),
...
create unique index if not exists trial_abuse_ip_hash_idx on public.trial_abuse (ip_address_hash);
...
create index if not exists stream_keys_user_id_idx on public.stream_keys (user_id);
...
create index if not exists embed_instances_user_id_idx on public.embed_instances (user_id);
...
drop trigger if exists subscription_tiers_set_updated_at on public.subscription_tiers;
...
drop trigger if exists embed_instances_set_updated_at on public.embed_instances;
...
drop policy if exists "Active tiers are publicly readable" on public.subscription_tiers;
...
-- stream_keys
drop policy if exists "Users manage own stream keys" on public.stream_keys;
...
-- embed_instances
drop policy if exists "Users manage own embeds" on public.embed_instances;
...
drop policy if exists "Admins manage trial abuse" on public.trial_abuse;
...
drop policy if exists "Admins manage domain whitelist" on public.domain_whitelist;
